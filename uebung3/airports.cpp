  ...
  for (auto& currentAirport : airportInfo) {
    ...
    if (currentAirport.second.m_routes.size() != 0) {
      std::transform(
        ...
        currentAirport.second.m_routes.end(),
        routeLengths.begin(),
        [&](std::pair <int, int> &routeFromCurrentAirport) {
        return calculateDistanceBetween(
          currentAirport.second.pos[0],
          currentAirport.second.pos[1],
          airportInfo[routeFromCurrentAirport.first].pos[0],
